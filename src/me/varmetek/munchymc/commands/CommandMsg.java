...
public class CommandMsg implements Element
{
  ...
  private CmdCommand[] commands = new CmdCommand[]{
    new CmdCommand.Builder("msg")
      .setLogic((sender,alias,args,len)->{
        ...
        if(len <=1){
          ...
          if((System.currentTimeMillis() - targUser.getLastMsgReply()) >= 2000){
            targUser.setMsgReply(pl.getName());
            targUser.setLastMsgReply(System.currentTimeMillis());
          }
          Messenger.send(pl,"&8&l TO "+ target.getName() + "&8=> &7"+ msg(Arrays.copyOfRange(args,1,len-1)));
          Messenger.send(target,"&8&l From "+ pl.getName() + "&8=>&7 "+ msg(Arrays.copyOfRange(args,1,len-1)));
        }
