 ...
 */
public class MineGeneratorImpl6 extends MineGenerator
{



  public MineGeneratorImpl6 (Mine mine){
   super(mine);
  }
   ...
   * **/
    private class RandomMaterial{
      ...
      private Map<LocalBlock,AtomicInteger> placed;
      ...
      public RandomMaterial ( List<LocalBlock> blocks , int volume){
        this.list = blocks;
        this.volume = volume;
        placed =  new HashMap<>(blocks.size());
      ...
      public MaterialData next(int sample){
        ...
        for(int i = 0; i< sample ;i++){
          ...
          for (LocalBlock data : list){
            ...
            double Vprop = placed.get(data).doubleValue() / ((double)volume);
            double Lprop = placed.get(data).doubleValue() / blockCount.doubleValue();

            if( data.getChance() > Vprop )continue;
            if(data.getChance() - ( Lprop - Vprop) < roll )continue;
